...
req2=Request('http://wdi.worldbank.org/table/3.1',headers={'User-Agent':'Chrome/101.0.4951.64'})
webpage1=urlopen(req2).read()
print('Webpage opened and read')
...
#Pulling country names
country_name_sqm_html1=[i for i in soup1.find_all(class_='country')]
country_name_sqm1=list()
for i in range (len(country_name_sqm_html1)): country_name_sqm1.append(country_name_sqm_html1[i].text)
print('country names acquired')

#Pulling country area
country_sqm_html1=[i for i in soup1.find_all(class_='spacer')]
country_sqm1=list()
for i in range (len(country_sqm_html1)): country_sqm1.append(country_sqm_html1[i].text)
print('country sqm acquired')
print(country_name_sqm1)

#this is a function to split a list based on position
def list_split(country_sqm1, n):
    for x in range(0, len(country_sqm1), n):
        every_chunk = country_sqm1[x: n+x]
...
print(list(list_split(country_sqm1, 14)))

#current
""" 
country_sqmdict1={'country name'  :country_name_sqm1}
print(country_sqmdict1)

area_sqmdict1={'country sqm'  :country_sqm1}
print(area_sqmdict1)

keys=area_sqmdict1.keys()
values=area_sqmdict1.values()

items = area_sqmdict1.items()

keys,values = zip(*area_sqmdict1.items())

print(keys)
print(values)

#dataframe1=pd.DataFrame.from_dict(area_sqmdict1)
#print(dataframe1)
